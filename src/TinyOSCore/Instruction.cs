...
public class Instruction
{
            ...
            for (var i = 1; i < g.Count; i++)
            {
                if (g[i].Value.Length != 0)
                {
                    ...
                    if (r.GroupNameFromNumber(i) == "param" || r.GroupNameFromNumber(i) == "const")
                    {
                        ...
                        if (uint.MaxValue == OpCodeParameter1)
                        {
                            OpCodeParameter1 = uint.Parse(g[i].Value);
                        }
                        else if (uint.MaxValue == OpCodeParameter2)
                        {
                            if (g[i].Value[0] == '-')
                            {
                                OpCodeParameter2 = (uint)int.Parse(g[i].Value);
                            }
                            else
                            {
                                OpCodeParameter2 = uint.Parse(g[i].Value);
                            }
                        }
                    }
                }
    ...
    /// </summary>
    public uint OpCodeParameter1 { get; set; } = uint.MaxValue;

    /// <summary>
    ///     The second parameter to the opCode.  May be a Constant or a Register value, or not used at all
    /// </summary>
    public uint OpCodeParameter2 { get; set; } = uint.MaxValue;
    ...
    {
        return
            $"OpCode: {(byte)OpCode,-2:G} {OpCode,-12:G}   Param1: {(OpCodeParameter1 == uint.MaxValue ? "" : OpCodeParameter1.ToString()),4:G}   Param2: {(OpCodeParameter2 == uint.MaxValue ? "" : OpCodeParameter2.ToString()),4:G}";
    }
}
